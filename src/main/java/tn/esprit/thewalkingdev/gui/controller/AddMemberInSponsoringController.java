...
public class AddMemberInSponsoringController extends Application implements
		Initializable {
	@FXML
		...
		for (Gamer s : listGamer) {
			if (table.getSelectionModel().getSelectedItem().getEmail()
				...
				JOptionPane pane = new JOptionPane(s.getFirstName() + " "
						+ s.getLastName() + "added to team sponsoring");
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			...
			for (Gamer s : listGamer) {
				...
				if (s.getTeam().getName().equals("team oganisation")) {
